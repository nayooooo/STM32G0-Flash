...
hw_uint8_t	 W25QXX_ReadSR(void);        		//读取状态寄存器 
void W25QXX_Write_SR(hw_uint8_t sr);  			//写状态寄存器
void W25QXX_Write_Enable(void);  		//写使能 
void W25QXX_Write_Disable(void);		//写保护
void W25QXX_Write_NoCheck(hw_uint8_t* pBuffer,hw_uint32_t WriteAddr,hw_uint16_t NumByteToWrite);
void W25QXX_Read(hw_uint8_t* pBuffer,hw_uint32_t ReadAddr,hw_uint16_t NumByteToRead);   //读取flash
void W25QXX_Write(hw_uint8_t* pBuffer,hw_uint32_t WriteAddr,hw_uint16_t NumByteToWrite);//写入flash
void W25QXX_Erase_Chip(void);    	  	//整片擦除
void W25QXX_Erase_Sector(hw_uint32_t Dst_Addr);	//扇区擦除
