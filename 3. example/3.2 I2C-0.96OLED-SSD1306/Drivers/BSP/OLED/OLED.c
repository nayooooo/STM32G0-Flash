 ...
 * @param y1 ¶ËµãÒ»µÄ×İ×ø±ê
 * @param x2 ¶Ëµã¶şµÄºá×ø±ê
 * @param y2 ¶Ëµã¶şµÄ×İ×ø±ê
...
void OLED_DrawLine(u8 x1,u8 y1,u8 x2,u8 y2,u8 mode)
{
	...
	uint8_t Point_Position_Inverse_Flag = 0X00;  // 0,1,2,3,4bit·Ö±ğ±íÊ¾µÚÒ»¡¢¶ş¡¢Èı¡¢ËÄÏóÏŞ¼°¹ØÓÚ y=x ¶Ô³Æ±êÊ¶
	...
	{
		//½« (x1, y1) ºÍ (x2, y2) ´¦Àí³ÉµÚÒ»ÏóÏŞÖĞµÄÇé¿ö
		//µÚÈıÏóÏŞ×ª»»Ö®ºó¿ÉÒÔ¿´³É±¾À´¾ÍÊÇÔÚµÚÒ»ÏóÏŞÖĞ
		if(dx>0 && dy>0) Point_Position_Inverse_Flag |= 0X01;
		else if(dx>0 && dy <0)				//´¦ÀíµÚ¶şÏóÏŞ
		{
			Point_Position_Inverse_Flag |= 0X02;
			dy = -dy;
			y2 = 2 * y1 - y2;
		}
		else if(dx<0 && dy < 0)			//´¦ÀíµÚÈıÏóÏŞ
		{
		...
		else if(dx<0 && dy >0)		//´¦ÀíµÚËÄÏóÏŞ
		{
			Point_Position_Inverse_Flag |= 0X08;
			dx = -dx;
			x2 = 2 * x1 - x2;
		}
		//½« (x1, y1) ºÍ (x2, y2) ´¦Àí³É 0<k<=1 µÄÇéĞÎ£¨×ö±ä»» { x0'=x1, y0'=y1 }£©
		if(dx<dy)
		{
			...
			x2 = x1 - y1 + temp.y;
			y2 = -x1 + y1 + temp.x;
			dx = x2 - x1; dy = y2 - y1;  // ÖØĞÂ¼ÆËã dx, dy È»ºóÔÙ´¦Àí³ÉµÚÒ»ÏóÏŞÖĞµÄÇé¿ö
		}
		...
		for(p.x++; p.x<=dx; p.x++)
		{
			if(d<0){			//´Ó¶«·½ÑÜÉú³öĞÂµÄÏñËØ
				d += incrE;
			}else{				//´Ó¶«±±·½ÑÜÉú³öĞÂµÄÏñËØ
				p.y++;
				d += incrNE;
			}
			
			//»æÖÆµã
			/* -- Ïà¶ÔÓÚµã (x0, y0) ÎªÔ­µãµÄ·´º¯ÊıÖ®×ø±ê --
			   -- x'= x0-y0+y --
			   -- y'=-x0+y0+x -- */
			switch(Point_Position_Inverse_Flag&0X0F)
			{
				case 0X01:		//µÚÒ»¡¢ÈıÏóÏŞ
				case 0X04:
...
/**
 * @brief OLED½Ç¶È»­Ïß£¨²ÎÊı²ÉÓÃ¼«×ø±ê±íÊ¾·¨£©
 * @param x0 ¼«µãµÄºá×ø±ê
 * @param y0 ¼«µãµÄ×İ×ø±ê
 * @param lenght Ö±ÏßµÄ¼«¾¶
...
/**
 * @brief OLEDï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ü´ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä»ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ò»ï¿½ï¿½ï¿½ï¿½ï¿½İ´ï¿½ï¿½ï¿½
...
{
	uint8_t Overflow_Flag = 0X00;  // 0, 1, 2, 3bit ï¿½Ö±ï¿½ï¿½Ê¾ï¿½ĞµÍ¸ï¿½Î»ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ĞµÍ¸ï¿½Î»ï¿½ï¿½ï¿½
	Point p1, p2;
...
/**
 * @brief OLED·ÅÖÃ 4 ¸öÏñËØ£¬ÒÔÖĞĞÄµãÎª×ø±êÔ­µã
 * @param x0 4¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 4¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 4¸öµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄºá×ø±ê
 * @param y 4¸öµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄ×İ×ø±ê
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
void OLED_Draw_4_Pixels(u8 x0, u8 y0, u8 x, u8 y, u8 mode)
{
	OLED_DrawPoint(x,		y,			mode);				/* µÚÒ»ÏóÏŞ */
	OLED_DrawPoint(x,		2*y0-y,		mode);				/* µÚ¶şÏóÏŞ */
	OLED_DrawPoint(2*x0-x,	2*y0-y,		mode);				/* µÚÈıÏóÏŞ */
	OLED_DrawPoint(2*x0-x,	y,			mode);				/* µÚËÄÏóÏŞ */
}

/**
 * @brief OLED·ÅÖÃ 2 Ìõ¾µÏñÏß£¬ÕâÁ½Ìõ¾µÏñÏß¹ØÓÚÖĞĞÄµã´¦µÄxÖá¶Ô³Æ
 * @param x0 4¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 4¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 4¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄºá×ø±ê
 * @param y 4¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄ×İ×ø±ê
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
...
/**
 * @brief OLED·ÅÖÃ 4 ¸öÈÆÖĞĞÄĞı×ªµÄÏñËØ£¬ÒÔÖĞĞÄµãÎª×ø±êÔ­µã
 * @param x0 4¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 4¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 4¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄºá×ø±ê
 * @param y 4¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄ×İ×ø±ê
 * @param angle Ïà¶ÔxÖáµÄĞı×ª½Ç¶È£¨ÄæÊ±Õë£©
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
// x' = xcosb - ysinb
// y' = xsinb + ycosb
// ¼ÆËã¶Ô³ÆµãÇ°ĞèÒª±ä»»Ò»ÏÂ×ø±ê£¬Òò´ËÊ¹ÓÃ int16_t Êı¾İÀàĞÍÒÔÂú×ã¸ºÊıºÍ·¶Î§ĞèÇó
void OLED_Draw_4_Pixels_Rotate(u8 x0, u8 y0, int16_t x, int16_t y, u16 angle, u8 mode)
{
	...
	//×¢Òâ¼ÓÉÏÖĞĞÄµãµÄ×ø±ê£¬ÒòÎª¼ÆËã½á¹ûÊÇÆ«ÒÆÁ¿
	OLED_DrawPoint(x_c-y_s		+x0,	x_s+y_c		+y0,	mode);					/* µÚÒ»ÏóÏŞ */
	OLED_DrawPoint(x_c-_y_s		+x0,	x_s+_y_c	+y0,	mode);					/* µÚ¶şÏóÏŞ */
	OLED_DrawPoint(_x_c-_y_s	+x0,	_x_s+_y_c	+y0,	mode);					/* µÚÈıÏóÏŞ */
	OLED_DrawPoint(_x_c-y_s		+x0,	_x_s+y_c	+y0,	mode);					/* µÚËÄÏóÏŞ */
}

/**
 * @brief OLED»­ 2 ÌõÈÆÖĞĞÄĞı×ª¾µÏñÏß£¬ÒÔÖĞĞÄµãÎª×ø±êÔ­µã
 * @param x0 4¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 4¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 4¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄºá×ø±ê
 * @param y 4¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚµÚÒ»ÏóÏŞÉÏµÄµãµÄ×İ×ø±ê
...
/**
 * @brief ÒÔÄ³Ò»µãÎªÖĞĞÄ»æÖÆ4¸öµã£¬Õâ4¸öµãºÍÖĞĞÄµãÖ®¼äµÄ¾àÀëÎª (dx, dy)
 * @details Æ«ÒÆ¾àÀëÊÇÏë¶ÁÓëÒ»ÇøÓòÉÏµÄµã¶øÑÔµÄ
 * @param x0 ¶Ô³ÆÖĞĞÄµÄºá×ø±ê
 * @param y0 ¶Ô³ÆÖĞĞÄµÄ×İ×ø±ê
 * @param x ĞèÒª»æÖÆµÄ4¸öµãÖĞ£¬Î»ÓÚÒ»ÇøÓòµÄµãµÄºá×ø±ê
 * @param y ĞèÒª»æÖÆµÄ4¸öµãÖĞ£¬Î»ÓÚÒ»ÇøÓòµÄµãµÄ×İ×ø±ê
 * @param dx Æ«ÒÆ¾àÀëµÄºáÖá·ÖÁ¿
 * @param dy Æ«ÒÆ¾àÀëµÄ×İÖá·ÖÁ¿
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
void OLED_Draw_4_Pixels_Spread_Out_From_Center(u8 x0, u8 y0, u8 x, u8 y, u8 dx, u8 dy, u8 mode)
{
	OLED_DrawPoint(x		+dx,	y		+dy,	mode);		/* µÚÒ»ÏóÏŞ */
	OLED_DrawPoint(x		+dx,	2*y0-y	-dy,	mode);		/* µÚ¶şÏóÏŞ */
	OLED_DrawPoint(2*x0-x	-dx,	2*y0-y	-dy,	mode);		/* µÚÈıÏóÏŞ */
	OLED_DrawPoint(2*x0-x	-dx,	y		+dy,	mode);		/* µÚËÄÏóÏŞ */
}

/**
 * @brief OLED·ÅÖÃ 8 ¸öÏñËØ
 * @param x0 8¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 8¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄºá×ø±ê
 * @param y 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄ×İ×ø±ê
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
/* --------------------------------------------------- *
 * Ò»	x = x,					y = y
 * ¶ş	x = y -y0+x0,			y = x-x0+y0
 * Èı	x = y-y0+x0,			y = 2*y0-(x-x0+y0)
 * ËÄ	x = x,					y = 2*y0-y
...
void OLED_Draw_8_Pixels(u8 x0, u8 y0, u8 x, u8 y, u8 mode)
{
	...
	OLED_DrawPoint(y+x0-y0,		x-x0+y0,		mode);				/* ¶ş */
	OLED_DrawPoint(y+x0-y0,		-x+x0+y0,		mode);				/* Èı */
...
/**
 * @brief OLED·ÅÖÃ 4 Ìõ¾µÏñÏß£¬Õâ 4 Ìõ¾µÏñÏß¹ØÓÚÖĞĞÄµã´¦µÄxÖá¶Ô³Æ
 * @details Ò»ÇøÓòÖĞµÄµãµÄÌØµãÊÇ£ºdy>dx>0
 * @param x0 8¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 8¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄºá×ø±ê
 * @param y 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄ×İ×ø±ê
...
/**
 * @brief ÒÔÄ³Ò»µãÎªÖĞĞÄ»æÖÆ8¸öµã£¬Õâ°Ë¸öµãºÍÖĞĞÄµãÖ®¼äµÄ¾àÀëÎª (dx, dy)
 * @details Æ«ÒÆ¾àÀëÊÇÏë¶ÁÓëÒ»ÇøÓòÉÏµÄµã¶øÑÔµÄ
 * @param x0 ¶Ô³ÆÖĞĞÄµÄºá×ø±ê
 * @param y0 ¶Ô³ÆÖĞĞÄµÄ×İ×ø±ê
 * @param x 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄºá×ø±ê
...
void OLED_Draw_8_Pixels_Spread_Out_From_Center(u8 x0, u8 y0, u8 x, u8 y, u8 dx, u8 dy, u8 mode)
{
	...
	OLED_DrawPoint(y+x0-y0	+dx,	x-x0+y0		+dy,	mode);		/* ¶ş */
	OLED_DrawPoint(y+x0-y0	+dx,	-x+x0+y0	-dy,	mode);		/* Èı */
...
/**
 * @brief OLED·ÅÖÃ 8 ¸öÈÆÖĞĞÄĞı×ªµÄÏñËØ£¬ÒÔÖĞĞÄµãÎª×ø±êÔ­µã
 * @param x0 8¸öµãµÄÖĞĞÄµÄºá×ø±ê
 * @param y0 8¸öµãµÄÖĞĞÄµÄ×İ×ø±ê
 * @param x 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄºá×ø±ê
 * @param y 8¸ö¶ËµãÖĞµÄÒ»¸öÎ»ÓÚÒ»ÇøÓòÉÏµÄµãµÄ×İ×ø±ê
...
void OLED_Draw_8_Pixels_Rotate(u8 x0, u8 y0, int16_t x, int16_t y, u16 angle, u8 mode)
{
	float rad = angle * (PI / 180);
	float sin_rad = sin(rad), cos_rad = cos(rad);
	//¼ÆËãĞı×ªºóµÄÆ«ÒÆÁ¿£¨ÏÂ·½ÕâĞ©²¢Ã»ÓĞÕæÕıËã³öÆ«ÒÆÁ¿£¬Ö»ÊÇ¼õÉÙ¼ÆËãÊ±¼ä£©£¬1±íÊ¾¸ººÅ£¬´óĞ´µÄ X,Y ÓÃÀ´Çø·Öºá×İ×ø±ê
	...
	OLED_DrawPoint( y_x_1y_X_c-x_1x_y_Y_s	+x0,	y_x_1y_X_s+x_1x_y_Y_c	+y0,	mode );				/* ¶ş */
	OLED_DrawPoint( y_x_1y_X_c-_x_x_y_Y_s	+x0,	y_x_1y_X_s+_x_x_y_Y_c	+y0,	mode );				/* Èı */
 ...
 * @brief OLED»­Ô²
 * @method brasenham·½·¨
 * @param x0 Ô²ĞÄµÄºá×ø±ê
 * @param y0 Ô²ĞÄµÄ×İ×ø±ê
 * @param r Ô²µÄ°ë¾¶
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
 ...
 * @brief OLED»­±ê×¼ÍÖÔ²
 * @method brasenham·½·¨
 * @param x0 ÍÖÔ²ÖĞĞÄµÄºá×ø±ê
 * @param y0 ÍÖÔ²ÖĞĞÄµÄ×İ×ø±ê
 * @param a ÍÖÔ²µÄ³¤°ëÖá³¤
 ...
 * @param x0 Á½¸öÍÖÔ²ÖĞĞÄµÄºá×ø±ê
 * @param y0 Á½¸öÍÖÔ²ÖĞĞÄµÄ×İ×ø±ê
 ...
 * @param a ÍÖÔ²µÄ³¤°ëÖá³¤
 * @param b ÍÖÔ²µÄ¶Ì°ëÖá³¤
 * @param angle Ïà¶ÔÓÚxÖáµÄĞı×ª½Ç¶È£¨ÄæÊ±Õë£©£¨½Ç¶ÈÖÆ£©
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
// x' = xcosb - ysinb
 ...
 * @param y BMPµÄ×óÉÏ½ÇµÄ¶¥µãµÄ×İ×ø±ê
 * @param p BMPÊı×éµÄÊ×µØÖ·
...
/**
 * @brief OLEDÌî³äÇø¿é
 * @details Ö´ĞĞÍê´Ëº¯ÊıºóÖ±½Ó¸üĞÂOLED
 * @param x1 Çø¿éµÄÒ»¸ö¶Ô½ÇÏßµÄ¶ËµãÒ»µÄºá×ø±ê
 * @param y1 Çø¿éµÄÒ»¸ö¶Ô½ÇÏßµÄ¶ËµãÒ»µÄ×İ×ø±ê
 * @param x2 Çø¿éµÄÒ»¸ö¶Ô½ÇÏßµÄ¶Ëµã¶şµÄºá×ø±ê
 * @param y2 Çø¿éµÄÒ»¸ö¶Ô½ÇÏßµÄ¶Ëµã¶şµÄ×İ×ø±ê
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
 ...
 * @param y ×Ö·ûµÄ×óÉÏ½ÇµÄ¶¥µãµÄ×İ×ø±ê
 * @param chr ĞèÒªÏÔÊ¾µÄ×Ö·û
 * @param size ×Ö·ûµÄ´óĞ¡£¬ÓĞ12¡¢15¡¢24¿ÉÑ¡
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
...
/**
 * @brief OLEDÏÔÊ¾Êı×Ö
 * @param x Êı×ÖµÄ×óÉÏ½ÇµÄ¶¥µãµÄºá×ø±ê
 * @param y Êı×ÖµÄ×óÉÏ½ÇµÄ¶¥µãµÄ×İ×ø±ê
 * @param num ĞèÒªÏÔÊ¾µÄÊı×Ö
 * @param len Êı×ÖµÄÎ»Êı
 * @param size Êı×ÖµÄ´óĞ¡£¬ÓĞ12¡¢15¡¢24¿ÉÑ¡
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
 ...
 * @param y ×Ö·û´®µÄ×óÉÏ½ÇµÄ¶¥µãµÄ×İ×ø±ê
 * @param p ĞèÒªÏÔÊ¾µÄ×Ö·û´®µÄÊ×µØÖ·
 * @param size ×Ö·û´®µÄ´óĞ¡£¬ÓĞ12¡¢15¡¢24¿ÉÑ¡
 * @param mode »æÖÆÄ£Ê½£¬FILL£¬Ìî³ä1£»CLEAR£¬Ìî³ä0
 * @return None
 */
